    ...
    <div className="auth-page flex min-h-dvh">
      <div className="flex w-full flex-col justify-between px-8 py-10 md:w-1/2 md:px-16 lg:px-24">
        ...
        <div className="mx-auto w-full max-w-sm">
          ...
          {result ? (
            <div className="mt-4 border-2 border-(--brand) bg-(--brand-soft) px-3 py-3 text-sm text-(--brand)">
              ...
              <p className="mt-2">{result.location.label}</p>
              <p className="text-xs text-(--brand)">
                {result.resolved.siteName ?? "SharePoint Site"} ·{" "}
                {result.resolved.driveName ?? "Drive"} · {result.resolved.rootName}
              </p>
              {result.location.webUrl ? (
                <a
                  href={result.location.webUrl}
                  target="_blank"
                  rel="noreferrer"
        ...
        <p className="text-center text-sm text-(--ink-muted)">
          This route replaces the old env-based MS365 allowlist flow.
        </p>
      </div>
